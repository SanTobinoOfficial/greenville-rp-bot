// API endpoint — przetwarzanie wynikow quizu weryfikacyjnego
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!token || !answers) {
      return NextResponse.json({ error: 'Brakujace dane' }, { status: 400 });
    }
    ...
    if (!quizToken) {
      return NextResponse.json({ error: 'Nieprawidlowy token' }, { status: 404 });
    }

    if (quizToken.used) {
      return NextResponse.json({ error: 'Token juz uzyty' }, { status: 400 });
    }
    ...
    for (let i = 0; i < Math.min(answers.length, questions.length); i++) {
      ...
      if (isCorrect) score++;
      results.push({ correct: isCorrect, correctAnswer: questions[i].correct, userAnswer: answers[i] });
    }

    const passed = score >= passScore;
    const partialPass = score >= 6 && score < passScore;

    await prisma.quizToken.update({ where: { token }, data: { used: true } });
    ...
    if (botApiUrl && botApiKey) {
      try {
        await fetch(`${botApiUrl}/quiz-result`, {
          ...
          body: JSON.stringify({ userId: quizToken.user.discordId, score, total: questions.length, passed, partialPass }),
        });
      } catch {}
    }
    ...
    return NextResponse.json({ score, total: questions.length, passed, partialPass, results });
